...
export default function AdminFilieres() {
  ...
  const load = () => filiereAPI.list().then(r => setFilieres(r.data)).finally(() => setLoading(false));
  useEffect(() => { load(); }, []);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-white">Filières</h1>
          <p className="text-slate-400 text-sm mt-1">{filieres.length} filière(s) configurée(s)</p>
        </div>
        <button onClick={openAdd}
          className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2.5 rounded-xl text-sm font-semibold transition-colors shadow-lg">
          <Plus className="w-4 h-4" />Ajouter une filière
        </button>
      </div>

      {loading ? (
        <div className="flex justify-center py-16"><div className="w-10 h-10 border-4 border-blue-400/30 border-t-blue-400 rounded-full animate-spin" /></div>
      ) : filieres.length === 0 ? (
        <div className="text-center py-16 bg-slate-800/30 rounded-2xl border border-white/10">
          <BookOpen className="w-12 h-12 text-slate-600 mx-auto mb-3" />
          <p className="text-slate-400">Aucune filière. Commencez par en ajouter une.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filieres.map((f, i) => (
            <motion.div key={f.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
              ...
              className="bg-slate-800/50 border border-white/10 rounded-xl overflow-hidden card-hover">
              <div className="h-2" style={{ background: f.couleur }} />
              <div className="p-5">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <div className="text-white font-bold">{f.nom}</div>
                    <div className="text-slate-400 text-xs mt-0.5 font-mono">{f.code}</div>
                  </div>
                  <div className="w-8 h-8 rounded-full" style={{ background: f.couleur + '33', border: `2px solid ${f.couleur}` }} />
                </div>
                {f.description && <p className="text-slate-400 text-xs mb-4 leading-relaxed">{f.description}</p>}
                ...
                <div className="flex gap-2">
                  <button onClick={() => openEdit(f)}
                    className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg bg-blue-500/20 text-blue-300 hover:bg-blue-500/30 transition-colors text-xs font-medium">
                    <Edit2 className="w-3.5 h-3.5" />Modifier
                  </button>
                  <button onClick={() => handleDelete(f)}
                    className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-red-500/20 text-red-300 hover:bg-red-500/30 transition-colors">
          ...
          <Modal title={editing ? 'Modifier la filière' : 'Nouvelle filière'} onClose={() => setShowModal(false)}>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              {[
                ...
                { label: 'Code *', key: 'code', placeholder: 'Ex: GL', upper: true },

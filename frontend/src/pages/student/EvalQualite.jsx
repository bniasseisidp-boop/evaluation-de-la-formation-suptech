...
import { Send, ChevronLeft, CheckCircle, Edit3 } from 'lucide-react';
...
export default function EvalQualite() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      setSubmitted(true);
      toast.success(existing ? 'Réponses mises à jour ! Merci 🙏' : 'Questionnaire soumis ! Merci 🙏');
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="bg-gradient-to-r from-blue-900 to-cyan-800 rounded-2xl p-6 text-white">
          ...
          <div className="mt-3 flex items-center gap-3 flex-wrap">
            <span className="bg-white/20 px-3 py-1 rounded-full text-sm">{answered}/{SERVICES.length} répondus</span>
            {existing && (
              <span className="bg-yellow-400/30 border border-yellow-300/40 px-3 py-1 rounded-full text-xs flex items-center gap-1">
                <Edit3 className="w-3 h-3" />Modification de votre réponse
              </span>
            )}
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        {SERVICES.map((service, i) => (
          <motion.div key={service.key}
            initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
            transition={{ delay: 0.04 * i }}
            className={`bg-white rounded-xl border p-5 transition-all ${values[service.key] ? 'border-green-200 shadow-sm' : 'border-slate-100'}`}>
            <div className="flex items-center gap-2 mb-4">
              <span className="text-2xl">{service.icon}</span>
              <span className={`font-bold text-sm ${service.highlight ? 'text-blue-700' : 'text-slate-800'}`}>
                {service.label}
              </span>
        ...
        <motion.button type="submit" disabled={submitting}
          ...
          {submitting
            ...
            : <><Send className="w-5 h-5" />{existing ? 'Mettre à jour mes réponses' : 'Soumettre le questionnaire'}</>}
        </motion.button>
      </form>
    </div>
  );

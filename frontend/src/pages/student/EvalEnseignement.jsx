...
import { cmpAPI, evaluationAPI } from '../../services/api';
...
const OPTIONS = [
  { value: 'A', label: 'A', score: '50%', color: 'text-red-600', bg: 'bg-red-50 border-red-200', check: 'border-red-500 bg-red-500' },
  { value: 'B', label: 'B', score: '75%', color: 'text-yellow-600', bg: 'bg-yellow-50 border-yellow-200', check: 'border-yellow-500 bg-yellow-500' },
  { value: 'C', label: 'C', score: '100%', color: 'text-green-600', bg: 'bg-green-50 border-green-200', check: 'border-green-500 bg-green-500' },
];
...
export default function EvalEnseignement() {
  ...
  useEffect(() => {
    cmpAPI.list().then(r => {
      ...
      setCmp(found || null);
    }).catch(() => toast.error('Matière introuvable')).finally(() => setLoading(false));
  }, [cmpId]);

  const onSubmit = async (data) => {
    setSubmitting(true);
    try {
      await evaluationAPI.submitEnseignement({ ...data, cmp_id: parseInt(cmpId) });
      setSubmitted(true);
  ...
  if (submitted) return (
    <motion.div initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
      ...
      <p className="text-slate-500 text-lg">Merci pour votre contribution. Redirection en cours...</p>
      <div className="mt-8 w-16 h-1 bg-gradient-to-r from-blue-600 to-cyan-500 rounded-full mx-auto animate-pulse" />
    </motion.div>
  );
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <button onClick={() => navigate('/portail')} className="flex items-center gap-2 text-slate-500 hover:text-blue-600 transition-colors text-sm mb-4">
        ...
        <div className="bg-gradient-to-r from-blue-900 to-blue-800 rounded-2xl p-6 text-white">
          ...
          <h1 className="text-2xl font-black">{cmp?.matiere?.nom}</h1>
          <div className="flex items-center gap-3 mt-2 flex-wrap">
            <span className="bg-white/20 px-3 py-1 rounded-full text-xs">👨‍🏫 {cmp?.professeur?.prenom} {cmp?.professeur?.nom}</span>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        {QUESTIONS.map(({ q, label }, i) => (
          <motion.div key={q} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
            ...
            <div className="flex gap-3">
              {OPTIONS.map(opt => (
                <label key={opt.value}
                  className={`flex-1 flex flex-col items-center gap-1.5 p-3 rounded-xl border-2 cursor-pointer transition-all duration-200 ${values[q] === opt.value ? `${opt.bg} border-opacity-100` : 'border-slate-200 hover:border-slate-300 hover:bg-slate-50'}`}>
                  <input type="radio" {...register(q, { required: 'Veuillez répondre à cette question' })}
                    value={opt.value} className="sr-only" />
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}
          ...
          <label className="text-slate-700 font-semibold text-sm mb-2 block">
            💬 Commentaire (optionnel)
          </label>
          <textarea {...register('commentaire')} rows={4}
            placeholder="Partagez vos remarques, suggestions ou commentaires sur cet enseignement..."
        ...
        <motion.button type="submit" disabled={submitting || answered < 10}
          ...
          className="w-full py-4 rounded-xl font-bold text-white bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-700 hover:to-cyan-600 disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-200 hover:scale-[1.01] shadow-lg shadow-blue-200 flex items-center justify-center gap-2">
          {submitting ? (
